...
const EditClass = () => {
    ...
    // Setupdate page initaal data data 
    const setData = (data) => {
        setName(data.name);
        if (data.course[0]) {
        ...
        if (data.teacher[0]) {
            setInstructor(data.teacher[0]);
        }

        setStartDateTimePicker(data.startDateTime);
        setEndDateTimePicker(data.endDateTime)
    }
    ...
    const updateClass = () => {
        ...
        // Api call
        axios.put(`${environment.base_url}/api/v1/classes/${state._id}`, data)
            .then(res => {
                navigate('/classes')
            }).catch(error => {
    ...
    // Get all teachers
    const getCourses = () => {
        console.log("the courses is called");
